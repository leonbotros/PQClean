...
void PQCLEAN_LEDAKEMLT52_LEAKTIME_gf2x_copy(DIGIT dest[], const DIGIT in[], size_t len) {
...
void PQCLEAN_LEDAKEMLT52_LEAKTIME_gf2x_mod_mul_dense_to_sparse(DIGIT *R, const DIGIT *dense,
    ...
    for (i = 0; i < n; i++) {
        ...
        PQCLEAN_LEDAKEMLT52_LEAKTIME_left_bit_shift_n(RLEN, w, ids); // shift inside digits by ids

        if (i == 0) {
            PQCLEAN_LEDAKEMLT52_LEAKTIME_gf2x_copy(tmpR, w, RLEN);
        } else {
            PQCLEAN_LEDAKEMLT52_LEAKTIME_gf2x_add(tmpR, tmpR, w, RLEN);
        }
    }
